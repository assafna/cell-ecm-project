...
def compute_fiber_densities(_simulations):
    ...
    with Pool(CPUS_TO_USE) as _p:
        for _keys, _value in tqdm(
            ...
            _fiber_densities[(_keys['simulation'], _keys['offset_x'], _keys['cell_id'], _keys['direction'])] = _value
        _p.close()
        _p.join()
    ...
    for _offset_x_index, _offset_x in enumerate(OFFSETS_X):
        ...
        for _simulation in tqdm(_simulations, desc='Simulations loop'):
            for _direction in ['inside', 'outside']:
                _left_cell_fiber_densities = _fiber_densities[(_simulation, _offset_x, 'left_cell', _direction)]
                _right_cell_fiber_densities = _fiber_densities[(_simulation, _offset_x, 'right_cell', _direction)]
