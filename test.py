...
def main():
    ...
    _roi = compute_lib.roi_by_microns(_resolution=0.41513, _length_x=CELL_DIAMETER_IN_MICRONS, _length_y=CELL_DIAMETER_IN_MICRONS / 2, _offset_x=0, _offset_y=0, _cell_coordinates=_cell_1_coordinates_new, _direction='right')
    ...
    for i in range(int(_roi[0]), int(_roi[2])):
        for j in range(int(_roi[1]), int(_roi[3])):
            ...
            rotated[j][i] = 255
    print('average', np.average(_values))
    plt.imshow(rotated)
    plt.show()
