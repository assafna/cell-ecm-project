...
def align_by_z_score(_experiments, _experiments_fibers_densities):
    _experiments_fibers_densities_aligned = {}
    for _tuple in tqdm(_experiments, desc='Temporal aligning experiments'):
        _experiment, _series_id, _group = _tuple

        _left_cell_alignment_fibers_densities = \
            _experiments_fibers_densities[(_experiment, _series_id, _group, 'left_cell', ALIGNMENT_OFFSET_Y)]
        _right_cell_alignment_fibers_densities = \
            _experiments_fibers_densities[(_experiment, _series_id, _group, 'right_cell', ALIGNMENT_OFFSET_Y)]

        _left_cell_fibers_densities = \
            _experiments_fibers_densities[(_experiment, _series_id, _group, 'left_cell', OFFSET_Y)]
        _right_cell_fibers_densities = \
            _experiments_fibers_densities[(_experiment, _series_id, _group, 'right_cell', OFFSET_Y)]
        ...
        for _time_frame, (_left_cell_fibers_density, _right_cell_fibers_density) in \
                enumerate(zip(_left_cell_alignment_fibers_densities, _right_cell_alignment_fibers_densities)):
            _normalized_left_cell_fibers_density = compute_lib.z_score(
                _x=_left_cell_fibers_density[0],
                _average=_normalization['average'],
                _std=_normalization['std']
            )
            _normalized_right_cell_fibers_density = compute_lib.z_score(
                _x=_right_cell_fibers_density[0],
                _average=_normalization['average'],
                _std=_normalization['std']
            )
            _mean_z_score = (_normalized_left_cell_fibers_density + _normalized_right_cell_fibers_density) / 2
            if _mean_z_score > START_TIME_FRAME_Z_SCORE:
                if _time_frame == 0 and BLOCK_ALREADY_DENSE:
                    ...
                    _left_cell_value_aligned = _left_cell_fibers_densities[_time_frame:]
                    _right_cell_value_aligned = _right_cell_fibers_densities[_time_frame:]
                break
        ...
        _experiments_fibers_densities_aligned[(_experiment, _series_id, _group, 'left_cell')] = \
            _left_cell_value_aligned
        _experiments_fibers_densities_aligned[(_experiment, _series_id, _group, 'right_cell')] = \
            _right_cell_value_aligned

    return _experiments_fibers_densities_aligned
...
def main(_high_time_resolution=True):
    ...
    _rois_dictionary, _rois_to_compute = \
        compute.rois(_arguments, _keys=['experiment', 'series_id', 'group', 'cell_id', 'offset_y'])
    ...
    _experiments_fibers_densities_aligned = align_by_z_score(_experiments, _experiments_fibers_densities)
            ...
            for _different_index in range(len(_experiment_tuples)):
                if _same_index != _different_index:
                    ...
                    for _same_cell_id, _different_cell_id in product(['left_cell', 'right_cell'],
                        ...
                        _same_fibers_densities = _experiments_fibers_densities_aligned[(
                            _same_experiment,
                            _same_series,
                            _same_group,
                            _same_cell_id
                        )]
                        _different_fibers_densities = _experiments_fibers_densities_aligned[(
    ...
    save.to_html(
        ...
        _filename='plot_band_' + str(BAND) + '_high_time_' + str(_high_time_resolution)
    )
